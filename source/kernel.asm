...
os_main:
	...
	;BootLogo
	mov ax, logo_bin_file_name
	call os_file_exists
	jc no_autorun_bin		; Skip next three lines if LOGO.BIN doesn't exist

	mov cx, 32768			; Otherwise load the program into RAM...
	call os_load_file
	jmp execute_logo_program		; ...and move on to the executing part
...
option_screen:
	...
	os_init_msg		db 'Welcome to CompOS', 0
	os_version_msg		db 'Version ', MIKEOS_VER, 0

	dialog_string_1		db 'Thanks for trying out CompOS!', 0
...
app_selector:
	...
	mov si, ax			; Did the user try to run 'LOGO.BIN'?
	mov di, logo_bin_file_name
	call os_string_compare
	jc no_logo_execute		; Show an error message if so
...
no_logo_execute:			; Warn about trying to executing kernel!
	mov ax, logodlg_string_1
	mov bx, logodlg_string_2
	mov cx, logodlg_string_3
	mov dx, 0			; One button for dialog box
	call os_dialog_box
...
not_bas_extension:
	...
	logo_bin_file_name	db 'LOGO.BIN', 0
	autorun_bas_file_name	db 'AUTORUN.BAS', 0

	bin_ext			db 'BIN'
	bas_ext			db 'BAS'

	kerndlg_string_1	db 'Cannot load and execute CompOS kernel!', 0
	kerndlg_string_2	db 'KERNEL.BIN is the core of CompOS, and', 0
	kerndlg_string_3	db 'is not a normal program.', 0

	logodlg_string_1	db 'Cannot load and execute CompOS logo!', 0
	logodlg_string_2	db 'LOGO.BIN is the logo of CompOS, and', 0
	logodlg_string_3	db 'is not a normal program.', 0

	ext_string_1		db 'Invalid filename extension! You can', 0
	ext_string_2		db 'only execute .BIN or .BAS programs.', 0

	basic_finished_msg	db '>>> CompOS BASIC program finished -- press a key', 0
